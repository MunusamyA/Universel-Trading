...
$documentTitle = trim((string)smt_pick($invoiceData, array('document_title', 'title', 'invoice_title'), 'Tax Invoice'));
if ($documentTitle === '') {
...
$pdf->AddPage();
$pdf->SetTitle($documentTitle . ' - ' . $invoiceNo);
...
$pdf->fitText($x0, 28.00, $x1 - $x0, 14.00, $documentTitle, 'C', 'B', 11.5);
// Outer border is drawn after dynamic invoice height is calculated.
...
/*
 * Keep Bank Details / Terms section compact.
 * Earlier this used $outerBottom = 820, so the bank/terms box stretched
 * to the page bottom and created a large empty area.
 */
$bankSectionHeight = 108.00;
$bankBottom = $bankTop + $bankSectionHeight;
$outerBottom = $bankBottom;

// Draw main outer box only after final dynamic bottom is known.
...
$pdf->fitText($x0 + 6, $bankTop + 26, $split - $x0 - 12, 10, $bankLine, 'L', '', 8.2);
$pdf->fitText($x0 + 6, $bankTop + 42, $split - $x0 - 12, 10, $bankAccountNo, 'L', '', 8.2);
$pdf->fitText($x0 + 6, $bankTop + 58, $split - $x0 - 12, 10, $bankIfsc, 'L', '', 8.2);
$pdf->fitText($x0 + 6, $bankTop + 74, $split - $x0 - 12, 10, $bankHolder, 'L', '', 8.2);
$pdf->fitText($split + 6, $bankTop + 8, $x1 - $split - 12, 10, 'Terms and Conditions', 'L', 'B', 8.4);
$pdf->blockText($split + 6, $bankTop + 26, $x1 - $split - 12, 50, $termsText, 'L', '', 8.2, 7.0);
...
$pdf->Output('I', preg_replace('/[^A-Za-z0-9_\-]/', '_', $documentTitle) . '_' . $safeInvoiceNo . '_no_overlap.pdf');
exit;
